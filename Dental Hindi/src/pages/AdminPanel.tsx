...
import { PlusCircle, MinusCircle, Save, Trash2, Edit2, X, MapPin } from 'lucide-react';
...
export function AdminPanel() {
  ...
  const [faqs, setFaqs] = useState<FAQ[]>([{ question: '', answer: '', videoId: '' }]);
  ...
  const fetchTreatments = async () => {
    try {
      ...
      console.error('इलाज प्राप्त करने में त्रुटि:', error);
    }
  };
  ...
  const getYouTubeId = (url: string) => {
    ...
    try {
      ...
      console.error('YouTube URL पार्स करते समय त्रुटि:', error);
      return '';
    }
  ...
  const handleEdit = (treatment: Treatment) => {
    ...
    setLocation(treatment.location || {
      building: '',
      floor: '',
      roomNumber: '',
      additionalInfo: ''
  ...
  const handleDelete = async (treatmentId: string) => {
    if (window.confirm('क्या आप वाकई इस इलाज को हटाना चाहते हैं?')) {
      try {
        await deleteDoc(doc(db, 'treatments', treatmentId));
        setMessage({ text: 'इलाज सफलतापूर्वक हटा दिया गया!', type: 'success' });
        fetchTreatments();
      } catch (error) {
        setMessage({ text: 'इलाज हटाने में त्रुटि', type: 'error' });
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!mainVideoId) {
        throw new Error('मुख्य वीडियो URL अमान्य है');
      }
      ...
      if (editingTreatment) {
        ...
        setMessage({ text: 'Treatment updated successfully!', type: 'success' });
      } else {
        await addDoc(collection(db, 'treatments'), {
          ...treatmentData,
          createdAt: new Date(),
        });
        setMessage({ text: 'इलाज सफलतापूर्वक अद्यतन किया गया!', type: 'success' });
      }
      ...
      setMessage({ 
        text: error.message || 'इलाज सहेजने में त्रुटि हुई. कृपया पुनः प्रयास करें.', 
        type: 'error' 
      });
    } finally {
    ...
    <AdminLayout>
      <div className="container mx-auto px-6 py-8">
        <div className="mb-8 flex justify-between items-center">
          <h1 className="text-3xl font-bold text-gray-800">इलाज प्रबंधन</h1>
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold">
                {editingTreatment ? 'इलाज संपादित करें' : 'नया इलाज जोड़ें'}
              </h2>
              <button
                onClick={resetForm}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setLocation({ ...location, additionalInfo: e.target.value })}
                      className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold">अक्सर पूछे जाने वाले प्रश्न</h3>
                  <button
                    type="button"
                    onClick={addFAQ}
                  ...
                  {faqs.map((faq, index) => (
                    <div key={index} className="p-4 border rounded-lg bg-gray-50">
                      <div className="flex justify-between items-start mb-4">
                        <h4 className="text-sm font-medium">अक्सर पूछे जाने वाले प्रश्न #{index + 1}</h4>
                        {faqs.length > 1 && (
                          <button
                            type="button"
                      ...
                      <div className="space-y-4">
                        ...
                        <input
                          type="url"
                          value={faq.videoId ? `https://youtube.com/watch?v=${faq.videoId}` : ''}
                          onChange={(e) => updateFAQ(index, 'videoId', e.target.value)}
              ...
              >
                ...
                <span>{isSubmitting ? 'सहेजा जा रहा है...' : editingTreatment ? 'इलाज अद्यतन करें' : 'इलाज जोड़ें'}</span>
              </button>
            </form>
          </div>
        ) : (
